...
import { DaemonLock, getDaemonInfo, getSocketPath } from './lock.js';
import { createIPCServer, IPCRequest } from './ipc.js';
import { Client } from '@modelcontextprotocol/sdk/client/index.js';
import { StdioClientTransport } from '@modelcontextprotocol/sdk/client/stdio.js';
import { getDaemonTimeoutMs } from '../config.js';
...
export async function startDaemon(
  ...
  const { generateDaemonIdWithEnv } = await import('./lock.js');
  const id = options.daemonId ?? generateDaemonIdWithEnv(command, args, options.env || {});
...
export class InProcessDaemon {
  ...
  async start(): Promise<void> {
    ...
    const { acquireDaemonLockWithEnv, generateDaemonIdWithEnv } = await import('./lock.js');
    const id = this.options.daemonId ?? generateDaemonIdWithEnv(this.command, this.args, this.options.env || {});

    // Acquire lock for this daemonId
    this.lock = await acquireDaemonLockWithEnv(this.command, this.args, this.options.env || {}, cwd, id);
